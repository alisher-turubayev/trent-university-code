 ...
 *  with First-Come-First-Serve (FCFS)
 * 
 ...
 * 
 * Dependencies:
 *  fcfs_queue.h - for implementation of FCFS queue.
 */
 ...
 *  them in the FCFS queue
 *
 * Arguments:
 *  *seed - int. Random seed for number generator
 *  *fcfs - struct queue. 
 */
void gen_file_request(struct queue *fcfs) {
...
int main() 
{
	...
	{
		gen_file_request(fcfs);
		
		while (queue_empty(fcfs) == FALSE) 
		{
			int new_track = queue_remove(fcfs);
