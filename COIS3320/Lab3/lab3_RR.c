...
/* Author: Richard Hurley. Modifications by Alisher Turubayev.			*/
/* Description:															*/
/*    This program simulates a single server queue with exponential		*/
/* interarrival time, exonential service time, and Round Robyn			*/
/* scheduling discipline. The parameter to the expon funciton is scaled */
/* by 100 to avoid problems with generating exponential variates.		*/
/* To turn the  debugging output off, change the constant DEBUG to 0	*/
/* and re-compile.														*/
/* **** NOTE: ALL CHANGES ARE MARKED WITH AT AT THE END OF COMMENDS		*/
/************************************************************************/
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
/* simulation constants - AT */
#define SIZE_QUANT 4	/* size of quantum - AT  */

/* simulation events */
#define ARRIVAL 0		/* arrival to queue 	 */
#define COMPLETE 1		/* completion of service */
#define QUANT 2			/* end of quantum - AT	 */
#define EOS 3			/* end of simulation 	 */
...
struct Custs{
	...
	long int remain_time;			/* remaining service time - AT */
};
...
long int cust_index;	/* customer index in the system - AT */

/* function declarations */
void arrive(struct event_node *ev_num);
void quant(struct event_node *ev_num);
...
void main()
{
	...
	while(not_done)
	{
		...
		/* update clock */
		clock = event->ev_time;
		/* process event type - AT */
		switch (event->ev_type)
		{
			case ARRIVAL  : arrive(event);
										break;
			case QUANT	  : quant(event);
										break;
...
/* Name: quant - AT													*/
/* Description														*/
/*    This function processes an end of quantum for a process. It	*/
/*	  performs following steps:										*/
/*	  1 - sets the server to idle.									*/
/* 	  2 - places the customer in a server queue.					*/
/*	  3 - if the queue is not empty, then start service.			*/
/********************************************************************/
void quant (struct event_node *ev_num)
{
...
#if DEBUG
	printf("quant function:\n");
	printf(" Customer taken out of queue because quantum ended.\n");
	printf(" Remaining time for service completion is %d\n\n", index->remain_time);
#endif

	/* put the customer n back in the queue */
	Puton_queue(&fcfs, index);
	/* start service back - we know that we have placed someone in the queue, so
...
void Gen_arrival(void)
{
	...
	/* initialize the remain_time as -1 - so that we can distinguish between a new and an old arrival - AT */
	index->remain_time = -1;
	
#if DEBUG
	printf("gen_arrival function:\n");
...
void Gen_departure(struct Custs *index)
{
	/* If the service time was not generated, generate one - AT */
	if (index->remain_time == -1) {
		long int time;
		/* generate exponential service time */
		time = expon(service_time);
		index->remain_time = time;
	}
	
#if DEBUG
	printf("gen_departure function:\n");
	printf(" Service time for customer is %d\n\n", index->remain_time);
#endif

	// If the service time is greater than quantum size, the customer
	// will remain in queue until completed
	if (SIZE_QUANT < index->remain_time) {
#if DEBUG
	printf(" quantum entered \n\n");
#endif
		index->remain_time -= SIZE_QUANT;
		Insert_event(QUANT, clock + SIZE_QUANT, index);
	}	
	else
		Insert_event(COMPLETE, clock + index->remain_time, index);
	return;
}
...
/*     2 - insert the node ar the end of the queue.              	 */
/*         2a - into an empty queue                             	 */
/*         2b - normal insertion                                	 */
/*********************************************************************/
void Puton_queue(struct Queue_struct *pqueue, struct Custs *pcust)
