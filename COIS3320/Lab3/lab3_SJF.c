...
/* interarrival time, exonential service time, and Shortest Job First	*/
...
#define FALSE 0
#define TRUE 1
#define DEBUG 1 /* set to 1 to turn debugging output on */
...
void arrive(struct event_node *ev_num)
{
	...
	/* generate service time (needed for priority sorting) - AT */
	index->service_time = expon(service_time);
...
{	
	...
	printf(" Completion of service is %d\n\n", clock + index->service_time);
#endif

	Insert_event(COMPLETE, clock + index->service_time, index);
	return;
}
...
void Puton_queue(struct Queue_struct *pqueue, struct Custs *pcust)
{
	...
	/**  ADDITION BEGIN  **/
	/* otherwise, put it according to the priority (shortest job first) */

	/* if the beginning of queue customer's service time is longer than the current
		one, place them at the beginning at the queue */
	if (((pqueue->q_head)->cust_index)->service_time >= pcust->service_time)
	{
	...
	{
		/* find the new place in the middle of the queue */
		int not_found = TRUE;
	
		struct Queue *curr = pqueue->q_head;
		while (curr->next != NULL && not_found) 
		{
			if (((curr->next)->cust_index)->service_time >= pcust->service_time) 
				not_found = FALSE;
			else
				curr = curr->next;
		}
		
		/* If inserting at the end of the queue */
		if (not_found) 
		{
